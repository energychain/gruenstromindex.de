const reqUpdates = {};
const getTrackerById = function(eventId) {
  let trackers = JSON.parse(window.localStorage.getItem("trackers"));
  for(let i=0;i<trackers.length;i++) {
    if(trackers[i].did.eventId == eventId) return trackers[i];
  }
...
const trackerRowHTML = function (tracker, fromDelegation) {
  let multipl = 1;
  let html = "";
  html += '<tr id="trackerRow' + tracker.did.eventId + '">';
  html += '<td>' + tracker.did.name + '</td>';
    html += '<td align="right">' + (multipl * tracker.did.consumption / 1000).toFixed(3).replace('.', ',') + '</td>';
    html += '<td align="right">' + (multipl * tracker.did.emission / 1000).toFixed(3).replace('.', ',') + '</td>';

  if (tracker.did.consumption !== 0) {
    html += '<td align="right">' + Math.round(1000 * multipl * (tracker.did.emission / tracker.did.consumption)) + '</td>';
  } else {
    html += '<td>&nbsp;</td>';
  }
  ...
  html += '<td title="'+new Date(tracker.did.iat*1000).toLocaleString()+'">' + niceTime(tracker.did.iat) + '</td>';
  html += '<td>';
  html += '<button title="Manuelle Ablesung" style="background-color:#147a50;margin-right:5px" class="btn btn-primary btn-sm btnReading" data-eventId="' + tracker.did.eventId + '" data-reading="'+tracker.did.reading+'">';
  ...
  html += '</button>';
  html += '<button title="Nachweisen" style="background-color:#147a50;margin-right:5px;" class="btn btn-primary btn-sm btnPresent" data-eventId="' + tracker.did.eventId + '">';
  ...
  html += '<button title="Teilen" style="background-color:#147a50;margin-right:5px;" disabled="disabled" class="btn btn-primary btn-sm btnShare" data-eventId="' + tracker.did.eventId + '">';
  ...
  html += '<button title="Inhaberschaft übertragen" style="background-color:#e6b41e;margin-right:5px;" class="btn btn-primary btn-sm btnTransferTracker" data-eventId="' + tracker.did.eventId + '">';
  ...
  html += '<button title="Löschen" style="background-color:#e6b41e;margin-right:5px;" class="btn btn-primary btn-sm btnRemoveTracker" data-eventId="' + tracker.did.eventId + '">';
...
const handleReadingButtonEvents = function () {
  ...
  $('.btnTransferTracker').on('click', function (e) {
    ...
    $('#transferTrackerFrm').on('submit', async function (e) {
        e.preventDefault();
  ...
  $('.btnReading').on('click', function (e) {
    let trackers = JSON.parse(window.localStorage.getItem("trackers"));
    for(let i=0;i<trackers.length;i++) {
      if(trackers[i].did.eventId == $(e.currentTarget).attr("data-eventId")) {
        $('#trackerName').html(trackers[i].did.name);
        $('#readingUpdate').val($(e.currentTarget).attr("data-reading") / 1000);
        $('#readingUpdate').attr('data-eventId', trackers[i].did.eventId);
        $('#readingUpdate').attr('min', $(e.currentTarget).attr("data-reading")  / 1000);
        $('#modalReading').modal('show');
      }
    }
  });
  $('.btnPresent').off();
  $('.btnPresent').on('click', async function (e) {
...
$(document).ready(function () {
  ...
  let trackers = window.localStorage.getItem("trackers");

  const fillTrackerContext = function() {
    let trackers = JSON.parse(window.localStorage.getItem("trackers"));
    if (trackers.length < 1) {
      ...
      let table = createTable(trackers);
      table += '<hr/>';
  ...
  if((typeof trackers !== 'undefined') && ( trackers !== null)) fillTrackerContext();

  const fetchTrackers = async function() {
    ...
    if(sourcesSig == null) {
        sourcesSig = await signJSON(startData);
        window.localStorage.setItem("sources_sig",JSON.stringify({sig:sourcesSig,iat:startData.iat}));
    }  
    fetch(url, {
        method: 'POST',
        headers: {
      ...
      .then(response => response.json())
      .then(data => { 
        window.localStorage.setItem("trackers",JSON.stringify(data));
        fillTrackerContext();
      });
  }
  fetchTrackers();
  ...
  $('#frmUpdateReading').on('submit', function (e) {
    ...
    let data = getTrackerById($('#readingUpdate').attr('data-eventId'));
